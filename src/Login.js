  ...
  return (
    <div className="login">
    	<form onSubmit={handleSubmit}>
			...
			<p><span>{signup ? 'Already have an account?' : 'New to MovieHub?'}{' '}</span><b style={signup ? {color: '#3cb19f'} : {color: '#ec215f'}} onClick={()=>{if(!loading){setSignup(!signup)}}}>Sign Up Now.</b></p>
		</form>
    </div>
  )
}
